class Pagination(object):
    ...
    def cleanpagepath(self, fullpath):
        ...
        if pos > -1:
            ...
            if querystring:
                pairs = [pair for pair in querystring.split('&') if
                         not pair.startswith('page') and not pair.startswith('hits')]
                if pairs:
    ...
    def pagination(self):
        ...
        if 0 < pages <= self.pagenum_per_bar:  # 总页数少于要显示的页数<< 1 2 >>
            ...
            if a > 3 and b < pages - 2:
                pagelist = [1, 0] + [t for t in range(self.page - int((self.pagenum_per_bar - 4) / 2),
                                                      self.page + int((self.pagenum_per_bar - 4) / 2) + 1)] + [0, pages]
            elif a <= 3:
